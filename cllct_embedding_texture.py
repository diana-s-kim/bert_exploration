...
#        activation={"last_hidden":np.empty((0,32,768))}#lst append
        activation={"last_hidden":[]}
        def getActivation(name):
                def hook(model, input, output):
#                    activation[name]=np.append(activation[name],output["last_hidden_state"].cpu().data.numpy(),axis=0)
                    activation[name].append(output["last_hidden_state"].cpu().data.numpy())# different length
                return hook
        bert.Qformer.bert.register_forward_hook(getActivation('last_hidden'))
        ...
        with open("./hidden_state/splited_txt_wo.txt","w") as f:
                for item in results:
                        f.write(" ".join(item)+"\n")
        f.close()

        ##filterin##
        with open("./hidden_state/output_attn_prob_wo.pkl","wb") as f:
